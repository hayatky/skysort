...
export function useReanalyzePhoto(jobId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (photoId: string) => api.reanalyzePhoto(photoId, { job_id: jobId, scope: "full" }),
    onSuccess: async () => {
...
export function useReanalyzeGroup(jobId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (groupId: string) => api.reanalyzeGroup(groupId, { job_id: jobId, scope: "full" }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["groups", jobId] });
      await queryClient.invalidateQueries({ queryKey: ["photos", jobId] });
    },
