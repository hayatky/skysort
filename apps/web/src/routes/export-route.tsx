      ...
      <div className="grid cols-2">
        <Panel title="XMP Write-back" copy="`dry_run=true` が既定です。">
          <div className="actions">
            <button className="button secondary" type="button" onClick={() => xmpExport.mutate({ jobId, dryRun: true })}>
              Run Dry-Run
            </button>
            <button className="button warning" type="button" onClick={() => xmpExport.mutate({ jobId, dryRun: false })}>
              Commit XMP
            </button>
          </div>
          {xmpExport.data ? (
