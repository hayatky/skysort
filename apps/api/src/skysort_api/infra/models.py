...
class Photo(Base):
    ...
    visual_features_json: Mapped[str] = mapped_column(Text, default="{}")
...
class Group(Base):
    ...
    boundary_reason: Mapped[str | None] = mapped_column(String, nullable=True)
    merge_suggested: Mapped[bool] = mapped_column(Boolean, default=False)
    merge_suggestion_reason: Mapped[str | None] = mapped_column(String, nullable=True)
...
class TechnicalScore(Base):
    ...
    sharpness_rank: Mapped[float | None] = mapped_column(Float, nullable=True)
    exposure_rank: Mapped[float | None] = mapped_column(Float, nullable=True)
    candidate_quality_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    reject_risk_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
...
class PhotoEvaluation(Base):
    ...
    problem_tags_json: Mapped[str] = mapped_column(Text, default="[]")
